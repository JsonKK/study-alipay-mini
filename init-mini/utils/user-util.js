...
var _authClient = function(app){
	...
	let deviceId = my.getStorageSync("temp_device_id");
	...
	let userActionParams = my.getStorageSync('userActionParams') || {channelQrcodeId:''};
...
var privateMethods = {
	...
	bindLocalPhone : function(app,msg,callback){
		let userInfo = appSession.getUserinfo();
		if(!userInfo || !msg){
			return;
		}
		...
		// 设置缓存
		my.setStorage(app.basicParams.APP_USERINFO_SESSION, userInfo);
